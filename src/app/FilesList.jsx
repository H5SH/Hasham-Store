...
const files = [
  ...
  {name: 'MovieBrowser', size: 'NA', type: '.apk', details: 'Work Under Progress', url: '', logo: '/logos/movieIcon.jpg'},
      ...
      {files.map((file, index) => (
        <Col md={4} key={index} className="mb-4">
          <a href={file.url} type={file.type} download={`${file.name}${file.type}`} className="file-download-link">
            <Card className="file-card" style={file.name === 'Pong' || file.name.includes('M') ? {color:'white', backgroundImage: `url(${file.logo})`}:{ backgroundImage: `url(${file.logo})`, 
